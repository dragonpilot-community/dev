...
class TestALKASettings(unittest.TestCase):
  ...
  def test_alka_setting_has_brands(self):
    ...
    # Brand -> safety model NAME, resolved lazily. Brands that arrive from a brand/*
    # branch (mg) have no SafetyModel at this layer, since brand branches are only
    # composed into 'full'; the full build still checks them.
    brand_to_safety_model = {
      "toyota": "toyota",
      "hyundai": "hyundai",
      "honda": "hondaNidec",
    ...
    for brand in alka_setting["brands"]:
      self.assertIn(brand, brand_to_safety_model, f"Unknown brand: {brand}")
      safety_mode = getattr(CarParams.SafetyModel, brand_to_safety_model[brand], None)
      if safety_mode is None:
        continue  # brand/* branch not composed into this one
